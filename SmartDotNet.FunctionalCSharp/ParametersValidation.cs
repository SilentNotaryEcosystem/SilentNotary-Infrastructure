    ...
    public static class ParametersValidation
    {
        ...
        public static Result NotNull(params (object value, string name)[] parameters)
        {
            ...
            foreach (var param in parameters)
            {
                Result res;
                if (param.value == null)
                    res = Result.Fail($"parameter {param.name} should be not null");
                else res = Result.Ok();
                results.Add(res);
            }
        ...
        {
            if (String.IsNullOrWhiteSpace(value))
                return Result.Fail($"parameter {name} should contain not empty string");
            return Result.Ok();
        }
        ...
        public static Result GreaterOrEqual(Int32 value, Int32 valueToCompare, string name)
        {
            if (value < valueToCompare)
                return Result.Fail($"parameter {name} should be greater or equal to {valueToCompare}");
            return Result.Ok();
        }
        ...
        {
            if (value == null)
                return Result.Fail($"parameter {name} should be not null");
            return Result.Ok();
        }
